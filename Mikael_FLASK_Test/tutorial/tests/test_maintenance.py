...
def test_logout(client):
    response = client.post('/logout') #Add URL
    assert response.status_code == 302

def test_acknowledge(client):
    response = client.post('/acknowledge') #Add URL
    assert response.status_code == 204

def test_alarm_report(client):
    response = client.post('/alarm_report') #Add URL
    assert response.status_code == 200

def test_operator_actions(client):
    response = client.post('/operator_actions') #Add URL
    assert response.status_code == 200

def test_acknowledge_history(client):
    response = client.post('/acknowledge_history') #Add URL
    assert response.status_code == 200

def test_alarm_history(client):
    response = client.post('/alarm_history') #Add URL
    assert response.status_code == 200

def test_maintenance(client):
    response = client.get('/maintenance') #Add URL
    assert response.status_code == 200
